const loadjs = {
  ...
  addFile: function (name, url) {
    if (!(name && url))
      return console.error("loadjs.addFile error : name / url not defined !");
    loadjs.files.push({ name: name, url: url });
  },
  addFiles: function (objs) {
    if (!Array.isArray(objs))
      return console.error(
        "loadjs.addFiles error : required an array of objs { name, url } !"
      );
  ...
  check4update: (objs) =>
    new Promise(async (res) => {
      ...
      for (let i = 0; i < objs.length; i++) {
        ...
        if (!js) continue;
        if (lsc == js) {
          console.log(obj.name, "is up-to-date");
          continue;
        }
        localStorage.setItem(fn, js);
        ufs.push(obj.name);
