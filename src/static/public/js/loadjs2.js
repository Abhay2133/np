...
function loadjs ( files = []) {
	...
	this.files = files;
	this.ufs = [];
	this.init = ( files = false, cb = () => null ) => new Promise (async res => {
	...
	this.getJs = ( name, url ) => new Promise ( async res => {
		log("getJs :", name);
	...
	this.c4u = (cb = () => null) => new Promise ( async res => {
		...
		for(let file of me.files) {
			...
			eval(js); localStorage.setItem(name, js);
			me.ufs.push(name);
		}
		let m = me.ufs.lengh > 0 ? "loadjs.c4u : updated files are" + JSON.stringify(me.ufs) : "All Files are Up-to-date !";
		log(m);
		res( cb() )
	})
}
...
let ljs = new loadjs (),
	files = [
	[ "test", "http://localhost:3000/js/test.js" ]
]
